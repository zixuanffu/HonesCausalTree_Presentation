...
---
### Final evaluation
1. Grow the tree again with the entire training set $S^{tr}$ and the selected $\alpha_k$ to get the final tree $\pi^*$. Construct the final estimator $\hat{\mu}_{\pi^*}(x;S^{tr})$.
...
3. X-learner
4. Transfrom the LHS variable from outcome into treatment effect
5. Transform the RHS variables.
6. 
   
---

1. Other splitting criteria: 
